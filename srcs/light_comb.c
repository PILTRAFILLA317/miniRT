...
/*   Updated: 2023/02/28 19:43:24 by umartin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	light_comb_disc(t_disc disc, t_elem *elem, t_vec rtn)
{
	...
	arg.dir = vec_norm(vec_diff(tmp->pos, rtn));
	arg.pos = rtn;
	if (i_w_disc(elem, arg, disc, *tmp) == 1)
		light = new_vec(0.0, 0.0, 0.0);
	tmp = tmp->next;
	while (tmp != NULL)
	{
