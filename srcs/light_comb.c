...
/*   Updated: 2023/02/28 19:05:27 by umartin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	light_comb_con(t_con con, t_elem *elem, t_vec rtn)
{
	...
	(void)arg;
	tmp = elem->light;
	t = vec_dot(vec_norm(points_to_vec(con.pos, rtn)),
			vec_norm(vec_diff(tmp->pos, rtn)));
	t = clamp(0, 1, t);
	t = t * (clamp(0, 1, (1 - (vec_len(points_to_vec(tmp->pos, rtn)))
					/ (tmp->bright * 1000))));
	light = (vec_mult(vec_mult_vec(col_to_01(con.color),
					col_to_01(tmp->color)), t * tmp->bright));
	// arg.dir = vec_norm(vec_diff(tmp->pos, rtn));
	// arg.pos = rtn;
	// if (inter_with_con(elem, arg, con, *tmp) == 1)
	// 	light = new_vec(0.0, 0.0, 0.0);
	tmp = tmp->next;
	while (tmp != NULL)
	{
		...
		t = t * (clamp(0, 1, (1 - (vec_len(points_to_vec(tmp->pos, rtn)))
						/ (tmp->bright * 1000))));
		aux = (vec_mult(vec_mult_vec(col_to_01(con.color),
						col_to_01(tmp->color)), t * tmp->bright));
		// arg.dir = vec_norm(vec_diff(tmp->pos, rtn));
		// arg.pos = rtn;
		// if (inter_with_con(elem, arg, con, *tmp) == 1)
